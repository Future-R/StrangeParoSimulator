...
export const SYSTEM_EVENTS: GameEvent[] = [
  ...
  },
  {
    ...
    操作指令: '属性变更 魅力 1; 属性变更 魅力 1 若 当前角色.标签组 存在 "歌手"; 属性变更 心情 10; 属性变更 精力 -10; 概率跳转 20 karaoke_after_h'
  },
  {
    id: 'karaoke_eat',
    权重: 0,
  ...
  // ===========================
  {
    ...
    // 效果：全员随机属性提升，双向增加友情
    操作指令: '列表执行(训练组, 属性变更 随机 10~30); 双向关系变更(友情, 变量.训练组, 5~15); 属性变更 心情 10'
  },
  ...
  // ===========================
  {
    ...
    可触发次数: -1,
    标签组: ['日常', '招募'],
    // 触发条件：训练员 && 有符合条件的马娘（马娘+非队友+友情>=60+无"不再考虑"标签）
    触发条件: '当前角色.标签组 存在 "训练员" && 存在角色满足(标签组 存在 "马娘" && 在队伍 == false && 关系.玩家.友情 >= 60 && 标签组 不存在 "不再考虑")',
    // 筛选符合条件的候选人
    预操作指令: '设置变量 列表 候选人 = 获取角色(非队友); 列表筛选(候选人, 标签组 存在 "马娘" && 关系.玩家.友情 >= 60 && 标签组 不存在 "不再考虑"); 设置变量 角色 目标 = 列表随机取值(候选人)',
    标题: '人才发掘',
  ...
  },
  {
    ...
    触发条件: 'false',
    标题: '任性的答应',
    正文: '“哼！既然你这么诚心诚意地求我了，本小姐就大发慈悲地加入你的队伍吧！”\n{目标.名称}虽然嘴上这么说，但身后疯狂摇摆的尾巴却暴露了她真实的心情。\n“要心怀感激地侍奉我哦，训练员！”',
